...
export default function SuratKeteranganPenyaksianTanahForm({
  isDialog,
}: {
  isDialog: boolean
}) {
  ...
  const { mutate: createSuratKeteranganPenyaksianTanah } = useMutation(
    trpc.suratKeteranganPenyaksianTanah.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil menyimpan surat keterangan penyaksian tanah",
        })
        await invalidateSuratKeteranganPenyaksianTanahsKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/surat/surat-keterangan-penyaksian-tanah")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal menyimpan surat keterangan penyaksian tanah")
      },
    }),
  )
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      createSuratKeteranganPenyaksianTanah({
        ...value,
        pendudukIds: [],
      })
    },
