...
export default function SuratKeteranganKelahiranForm({
  ...
  const suratKeteranganKelahiranByIdKey =
    trpc.suratKeteranganKelahiran.byId.queryKey(id)
  ...
  const { mutate: updateSuratKeteranganKelahiran } = useMutation(
    trpc.suratKeteranganKelahiran.update.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil memperbaharui surat keterangan kelahiran",
        })
        await queryClient.invalidateQueries({
          queryKey: suratKeteranganKelahiransKey,
        })
        await queryClient.invalidateQueries({
          queryKey: suratKeteranganKelahiranByIdKey,
        })
        if (isDialog) {
          router.back()
        } else {
          router.push("/surat/skl")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal memperbaharui surat keterangan kelahiran")
      },
    }),
  )
  ...
  React.useEffect(() => {
    if (data) {
      if (data.nikAyah) {
        const ayahLabel = `${data.nikAyah} - ${data.namaAyah}`
        setSelectedAyah({
          label: ayahLabel,
          value: data.nikAyah,
          nama: data.namaAyah,
          alamat: data.alamatAyah,
          tempatLahir: data.tempatLahirAyah,
          tanggalLahir: data.tanggalLahirAyah ?? new Date(),
          wilayah: data.alamatWilayahAyah,
          pekerjaan: data.pekerjaanAyah,
        })
      }
      if (data.nikIbu) {
        const ibuLabel = `${data.nikIbu} - ${data.namaIbu}`
        setSelectedIbu({
          label: ibuLabel,
          value: data.nikIbu,
          nama: data.namaIbu,
          alamat: data.alamatIbu,
          tempatLahir: data.tempatLahirIbu,
          tanggalLahir: data.tanggalLahirIbu ?? new Date(),
          wilayah: data.alamatWilayahIbu,
          pekerjaan: data.pekerjaanIbu,
        })
      }
      if (data.nikPemohon) {
        const pemohonLabel = `${data.nikPemohon} - Pemohon`
        setSelectedPemohon({
          label: pemohonLabel,
          value: data.nikPemohon,
          nama: "Pemohon",
          alamat: "Alamat Pemohon",
          tempatLahir: "Tempat Lahir Pemohon",
          tanggalLahir: new Date(),
          wilayah: "Wilayah Pemohon",
        })
      }
    }
  }, [data])
  ...
  const defaultValues = React.useMemo<z.input<typeof formSchema>>(
    () => ({
      ...
      jenisKelaminAnak: data?.jenisKelaminAnak ?? "laki-laki",
      nikAyah: data?.nikAyah ?? "",
      namaAyah: data?.namaAyah ?? "",
      tempatLahirAyah: data?.tempatLahirAyah ?? "",
      tanggalLahirAyah: data?.tanggalLahirAyah ?? "",
      pekerjaanAyah: data?.pekerjaanAyah ?? "belum_tidak bekerja",
      alamatAyah: data?.alamatAyah ?? "",
      alamatWilayahAyah: data?.alamatWilayahAyah ?? "",
      nikIbu: data?.nikIbu ?? "",
      namaIbu: data?.namaIbu ?? "",
      tempatLahirIbu: data?.tempatLahirIbu ?? "",
      tanggalLahirIbu: data?.tanggalLahirIbu ?? "",
      pekerjaanIbu: data?.pekerjaanIbu ?? "belum_tidak bekerja",
      alamatIbu: data?.alamatIbu ?? "",
      alamatWilayahIbu: data?.alamatWilayahIbu ?? "",
