...
export default function SuratKeteranganKepemilikanRumahForm({
  isDialog,
}: {
  isDialog: boolean
}) {
  ...
  const { mutate: createSuratKeteranganKepemilikanRumah } = useMutation(
    trpc.suratKeteranganKepemilikanRumah.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil menyimpan surat keterangan kepemilikan rumah",
        })
        await invalidateSuratKeteranganKepemilikanRumahsKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-keterangan-kepemilikan-rumah")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal menyimpan surat keterangan kepemilikan rumah")
      },
    }),
  )
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      createSuratKeteranganKepemilikanRumah(value)
    },
  })
