...
export default function SuratKeteranganPenghasilanForm({
  isDialog,
}: {
  isDialog: boolean
}) {
  ...
  const { mutate: createSuratKeteranganPenghasilan } = useMutation(
    trpc.suratKeteranganPenghasilan.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil menyimpan surat keterangan penghasilan",
        })
        await invalidateSuratKeteranganPenghasilansKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-keterangan-penghasilan")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal menyimpan surat keterangan penghasilan")
      },
    }),
  )
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      createSuratKeteranganPenghasilan(value)
    },
  })
