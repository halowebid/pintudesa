...
export default function SuratKeteranganPenghasilanOrangTuaForm({
  isDialog,
}: {
  isDialog: boolean
}) {
  ...
  const { mutate: createSuratKeteranganPenghasilanOrangTua } = useMutation(
    trpc.suratKeteranganPnghasilanOrangTua.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description:
            "Berhasil menyimpan surat keterangan penghasilan orang tua",
        })
        await invalidateSuratKeteranganPenghasilanOrangTuasKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-keterangan-penghasilan-orang-tua")
        }
      },
      onError: (error) => {
        handleError(
          error,
          "Gagal menyimpan surat keterangan penghasilan orang tua",
        )
      },
    }),
  )
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      createSuratKeteranganPenghasilanOrangTua({
        ...value,
        pendudukIds: [],
      })
    },
