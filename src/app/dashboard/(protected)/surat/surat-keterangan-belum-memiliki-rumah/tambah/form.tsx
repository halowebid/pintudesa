...
export default function SuratKeteranganBelumMemilikiRumahForm({
  isDialog,
}: {
  isDialog: boolean
}) {
  ...
  const { mutate: createSuratKeteranganBelumMemilikiRumah } = useMutation(
    trpc.suratKeteranganBelumMemilikiRumah.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description:
            "Berhasil menyimpan surat keterangan belum memiliki rumah",
        })
        await invalidateSuratKeteranganBelumMemilikiRumahsKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-keterangan-belum-memiliki-rumah")
        }
      },
      onError: (error) => {
        handleError(
          error,
          "Gagal menyimpan surat keterangan belum memiliki rumah",
        )
      },
    }),
  )
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      createSuratKeteranganBelumMemilikiRumah(value)
    },
  })
