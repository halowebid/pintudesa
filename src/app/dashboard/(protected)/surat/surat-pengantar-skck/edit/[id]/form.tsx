...
export default function SuratPengantarSKCKForm({
  ...
  const suratPengantarSKCKByIdKey = trpc.suratPengantarSKCK.byId.queryKey(id)
  ...
  const { mutate: updateSuratPengantarSKCK } = useMutation(
    trpc.suratPengantarSKCK.update.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil memperbaharui surat pengantar SKCK",
        })
        await queryClient.invalidateQueries({
          queryKey: suratPengantarSKCKsKey,
        })
        await queryClient.invalidateQueries({
          queryKey: suratPengantarSKCKByIdKey,
        })
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-pengantar-skck")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal memperbaharui surat pengantar SKCK")
      },
    }),
  )
