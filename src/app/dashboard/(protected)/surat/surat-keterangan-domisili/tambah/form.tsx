...
export default function SuratKeteranganDomisiliForm({
  ...
  const { mutate: createSuratKeteranganDomisili } = useMutation(
    trpc.suratKeteranganDomisili.create.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil membuat surat keterangan domisili",
        })
        await invalidateSuratKeteranganDomisilisKey()
        if (isDialog) {
          router.back()
        } else {
          router.push("/dashboard/surat/surat-keterangan-domisili")
        }
      },
      onError: (error) => {
        handleError(error, "Gagal membuat surat keterangan domisili")
      },
    }),
  )
