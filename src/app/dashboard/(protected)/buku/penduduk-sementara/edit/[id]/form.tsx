...
export default function PendudukSementaraForm({
  ...
  const pendudukSementaraByIdKey = trpc.pendudukSementara.byId.queryKey(id)
  const invalidatePendudukSementaraByIdKey = async () => {
    await queryClient.invalidateQueries({ queryKey: pendudukSementaraByIdKey })
  }
  ...
  const { data } = useQuery(trpc.pendudukSementara.byId.queryOptions(id))
  const { mutate: updatePendudukSementara } = useMutation(
    trpc.pendudukSementara.update.mutationOptions({
      onSuccess: async () => {
        toast({
          description: "Berhasil memperbaharui penduduk sementara",
        })

        await invalidatePendudukSementaraByIdKey()
      ...
      onError: (error) => {
        handleError(error, "Gagal memperbaharui penduduk sementara")
      },
    }),
  )
  ...
  const defaultValues = React.useMemo<z.input<typeof formSchema>>(
    () => ({
      ...data,
      nama: data?.nama ?? "",
      nomorIndentitas: data?.nomorIndentitas ?? "",
      jenisKelamin: data?.jenisKelamin ?? "laki-laki",
      tempatLahir: data?.tempatLahir ?? "",
      tanggalLahir: data?.tanggalLahir ?? "",
      pekerjaan: data?.pekerjaan ?? "lainnya",
      kebangsaan: data?.kebangsaan ?? "wni",
      keturunan: data?.keturunan ?? "",
      datangDari: data?.datangDari ?? "",
      tujuanKedatangan: data?.tujuanKedatangan ?? "",
      namaYangDidatangi: data?.namaYangDidatangi ?? "",
      alamatYangDidatangi: data?.alamatYangDidatangi ?? "",
    ...
    onSubmit: ({ value }) => {
      updatePendudukSementara({
        ...
        tanggalLahir: value.tanggalLahir
          ? formatStringToDate(value.tanggalLahir)
          : undefined,
        tanggalDatang: value.tanggalDatang
          ? formatStringToDate(value.tanggalDatang)
          : undefined,
